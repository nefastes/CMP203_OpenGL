...
protected:
	...
	std::vector<float> invertedNormals;	//Will hold the opposite values for of normal so that when we render the inside of the shape we get proper lighting
...
public:
	...
	void render(short unsigned textureFilteringMode = 3, bool renderAsShadow = false);
